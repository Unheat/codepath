import { Link } from "react-router-dom";

const Card = (props) => {
  return (
    <article className="creator-card text-[#3D5566] border border-[#0D548E] bg-[#425D73]">
      {/*if iamge exst then show it up */}
      {props.imageURL && (
        <img
          src={props.imageURL}
          alt={props.name}
          style={{ width: "100%", borderRadius: "8px" }}
        />
      )}

      <div className="card-content">
        <h3>{props.name}</h3>

        <div
          className="info-edit h-[46px] rounded-[40px] flex-row flex text-center justify-end relative top-auto right-auto bottom-auto left-auto w-[607px]"
          style={{ display: "flex", gap: "10px" }}
        >
          <Link to={`/view/${props.id}`}>
            <img src="/icons/info.svg" alt="info" style={{ width: "24px" }} />
          </Link>
          <Link to={`/edit/${props.id}`}>
            <img src="/icons/pen.svg" alt="pen" style={{ width: "24px" }} />
          </Link>
        </div>
        <div className="social-rows" style={{ display: "flex", gap: "10px" }}>
          {/*only show the media icon if that the databse have */}
          {props.youtube && (
            <a href={props.youtube} target="_blank" rel="noreferrer">
              <img
                src="/icons/youtube.svg"
                alt="YouTube"
                style={{ width: "24px" }}
              />
            </a>
          )}

          {props.twitter && (
            <a href={props.twitter} target="_blank" rel="noreferrer">
              <img
                src="/icons/twitter.svg"
                alt="Twitter"
                style={{ width: "24px" }}
              />
            </a>
          )}

          {props.instagram && (
            <a href={props.instagram} target="_blank" rel="noreferrer">
              <img
                src="/icons/instagram.svg"
                alt="Instagram"
                style={{ width: "24px" }}
              />
            </a>
          )}
        </div>
        <p>{props.description}</p>
      </div>
    </article>
  );
};
export default Card;
