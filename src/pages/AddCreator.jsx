...
const AddCreator = () => {
  ...
  const addCreator = async (event) => {
    ...
    const { error } = await supabase.from("creators").insert([finalCreator]);
    if (error) {
  ...
  return (
    <div className="AddCreator m-[10px] mr-[30px] ml-[30px]">
      <h2 className="text-center">Add a New Creator</h2>
      ...
      >
        ...
        <button type="submit" className="w-[888px]">
          Submit Creator
        </button>
      </form>
    </div>
